...
impl Solution {
    pub fn is_palindrome(s: String) -> bool {
        ...
        let (mut l, mut r) = (0 as usize, chars.len() - 1);
    ...
    pub fn valid_palindrome(s: String) -> bool {
        let chars: Vec<char> = s.chars().collect();
        let (mut l, mut r) = (0 as usize, chars.len() - 1);
        if chars == chars.iter().rev().cloned().collect::<Vec<char>>() {
            return true;
        }

        while l < r {
            if chars[l] != chars[r] {
                let left_skipped = chars[..l].iter().chain(chars[l+1..].iter()).collect::<String>();
                let right_skipped = chars[..r].iter().chain(chars[r+1..].iter()).collect::<String>();
                return Solution::is_palindrome(left_skipped) || Solution::is_palindrome(right_skipped);
            }
            l += 1;
            r = r.saturating_sub(1);
        }
...
fn main() {
    #[cfg(debug_assertions)]
    {
        ...
        for _ in 0..test_cases {
            ...
            let ans = Solution::valid_palindrome(s);
