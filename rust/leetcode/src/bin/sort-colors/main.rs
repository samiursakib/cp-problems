...
impl Solution {
    pub fn sort_colors(nums: &mut Vec<i32>) {
        let (mut l, mut m, mut r) = (0, 0, nums.len().saturating_sub(1));
...
fn main() {
    #[cfg(debug_assertions)]
    {
        ...
        for _ in 0..test_cases {
            ...
            let mut nums = read!(lines.next().unwrap(), [i32]);
            let ans = Solution::sort_colors(&mut nums);
            writeln!(writer, "{:?}", nums).unwrap();
        }
    }
}
